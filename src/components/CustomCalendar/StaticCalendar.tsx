...
const StaticCalendar: React.FC<CalendarProps> = ({
  startDay = new Date(),
  allSelectedDates,
  mySelectedDates,
  ...
  const isMySelectedDates = (date: Date) => {
  ...
  return (
    <StyledCalendarWrapper>
      <Calendar
        ...
        formatMonthYear={(_, date) => date.toLocaleDateString('en-US', { month: 'long' })}
        ...
        minDate={MIN_DATE}
        maxDate={MAX_DATE}
        onClickDay={(date) => onClick(date)}
        tileClassName={({ date }) => {
          if (date.toDateString() === clickedDate?.toDateString()) return 'greenish-selected';
          if (isMySelectedDates(date)) return 'selected';
          if (isSelectedDate(date)) return 'grayish-selected';
          return '';
        }}
      />
    </StyledCalendarWrapper>
